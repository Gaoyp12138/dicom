...
//	public DisplayTag(List<File> files){
//		try {
//			setObject(loadDicomObject(files));
//		} catch (IOException e) {
//			Logger.getLogger(DisplayTag.class.getName()).log(Level.SEVERE, null, e);
//		}
//	}
	...
	/**
	* @Author: GaoYanping
	* @Description: 重写 loadDicomObject 上传文件列表
	* @Date: 15:32 2017/10/24
	*/

//	public Attributes loadDicomObject(List<File> files) throws IOException {
//		if (files == null){
//			return null;
//		} else {
...
//	}
	...
	/**
	 * Permet d'afficher l'heure d'une valeur dicom en standard international
	 * yyyy.mm.dd/ Permit display time in format yyyy.mm.dd
	 * 
	 * @param Tag
	 *            : valeur du tag / int tag
	 * @param valueBool
	 *            : si true Format yyyy.mm.dd sinon format dd.mm.yyyy/ if true
	 *            then format yyyy.mm.dd else dd.mm.yyyy
	 * @param valueNoun
	 *            : "dot" mettre la date en format yyyy.mm.dd ou dd.mm.yyyy
	 *            sinon en format yyyy mm dd ou dd mm yyyy/ "dot" put yyyy.mm.dd
	 *            or dd.mm.dd or dd.mm.yyyy else yyyy mm or dd mm yyyy
	 * @return afficher le string du tag selon le standard international/ return
	 *         string Date
	 * @throws IOException
	 */
	...
	/**
	 * Permet d'afficher l'heure d'une valeur dicom en standard international
	 * yyyy.mm.dd/ Permit display a time in metadata for yyyy.mm.dd
	 * 
	 * @param object
	 * @param Tag
	 *            : valeur du tag/ value of tag
	 * @param valueBool
	 *            : si true Format yyyy.mm.dd sinon format dd.mm.yyyy/ if true
	 *            format yyyy.mm.dd else dd.mm.yyyy
	 * @param valueNoun
	 *            : "dot" mettre la date en format yyyy.mm.dd ou dd.mm.yyyy
	 *            sinon en format yyyy mm dd ou dd mm yyyy/dot" put yyyy.mm.dd
	 *            or dd.mm.dd or dd.mm.yyyy else yyyy mm or dd mm yyyy
	 ...
	 *            : String date
	 * @param valueBool
	 *            : if true Format yyyy.mm.dd else format dd.mm.yyyy
	 * @param valueNoun
	 *            : "dot" put the date in format yyyy.mm.dd or dd.mm.yyyy else
	 *            in format yyyy mm dd or dd mm yyyy
	 * @return
	 */
	public static String FormatDate(String Numero, boolean valueBool, String valueNoun) {
		if (Numero.matches("^[0-9]*$")) {// If la chaine de caractère est un
											// nombre ou un chiffre
			...
			if (valueBool == true) {// Format yyyy.mm.dd
				...
				for (int i = 4, j = 6; i < j; i++) {
					r.append(Numero.charAt(i));// The first char value of the
												// sequence is at index zero,
												// the next at index one, and so
	 ...
	 * 
	 * If use setDicomObject(readDicomObject(File f)), and
	 * getHeaderDateValue(getDicomObject())
	 * 
	 * @param tagNr
	 *            "0000,0010"
	 * @return
	...
	/**
	 * Converts the string representation of a header number e.g. 0008,0010 to
	 * the corresponding integer as 0x00080010 as used in the @see
	...
	/**
	 * Convertor tag to String Using VM !=1 example result [25, 25]
	 * 
	 * @param Tag
	 * @return
	 */
	public static String getStringTag(Attributes object, int Tag) {
		String tagValue2[] = object.getStrings(Tag);// Conversion table in List
													// to String
	...
	/**
	 * Permit display time in hh.mm.ss (0008,0030) AT S Study Time (0008,0031)
	 * AT S Series Time (0008,0032) AT S Acquisition Time (0008,0033) AT S Image
	 * Time
	 * 
	 * @param //file
	 *            : input file
	...
	/**
	 * Permit display time in hh.mm.ss.fac (0008,0030) AT S Study Time
	 * (0008,0031) AT S Series Time (0008,0032) AT S Acquisition Time
	...
	/**
	 * Permit display time in hh.mm.ss.frac (0008,0030) AT S Study Time
	 * (0008,0031) AT S Series Time (0008,0032) AT S Acquisition Time
	 * (0008,0033) AT S Image Time
	 * 
	 * @param object
	 ...
	 * 
	 * @param //headerNr
	 *            e.g. "0018,0050" to get Slice Thickness<br>
	 *            or "0008,0102#0054,0220" to get the Coding Scheme Designator
	 *            after View Code Sequence
	 * @return int
	 * @throws //DicomHeaderParseException
	 */
	...
	/**
	 * returns the String representation of the given header field if it exists
	 * in the header
	 * 
	 * @param tagNr
	 * @return
	...
	private static String toElementString(String dcmele, int tag) {
		...
		StringBuffer append = sb.append(TAG).append(" [").append(getObject().getVR(tag)).append("] ")
				.append(object.tags()).append(": ").append(dcmele);
		return sb.toString();
	}
	...
	/**
	 * Giving power Example: setFactorPower(10,2)//10^2
	 * 
	 * @param result3
	 * @param factor
	 * @return
	 ...
	 * 
	 * @param object
	 * @param PATIENT_ADDITIONAL_TAGS
	 *            : Table int
	 * 
	 *            example : public static final int[] tag = { 0x00080020,
	 *            0x00080022, };
	 * 
	 *            FileInputStream fis = new FileInputStream(fileInput);
	 *            DicomInputStream dis = new DicomInputStream(fis); DicomObject
	 *            obj = dis.readDicomObject(); String nounValue[]
	 *            =getValue(obj,tag);
	 * 
	 * @return
	 */
	...
	/**
	 * Les unités spécifiques selon les tags pour vr= SQ/ Unity specical for
	 * tags VR= SQ
	 * 
	 * @param TAG
	...
	public void unit(int TAG, String result) {
		...
		} else if (TAG == Tag.PhysicalUnitsXDirection || TAG == Tag.PhysicalUnitsXDirection
	...
	/**
	 * Special Ratio Spatial toutes les unites sont en mm/ Giving tag ratio
	 * Spatial of mm
	 * 
	 * @param TAG
	 *            : entree choisi - PhysicalUnitsXDirection -
	 *            PhysicalUnitsYDirection -PixelComponentPhysicalUnits
	 * 
	 * @param result:
	 *            prend l'unite
	 */
	public void unitRatioSpatial(int TAG, String result) {
		if (TAG == Tag.PhysicalUnitsXDirection || TAG == Tag.PhysicalUnitsYDirection
				|| TAG == Tag.PixelComponentPhysicalUnits) {
			val2 = Integer.valueOf(result).intValue();// convertie en int
	 ...
	 * @param tagNr
	 *            the Tag to read
	 * @return the value as String Returns the Specific Character Set defined by
	 *         Attribute Specific Character Set (0008,0005) of this or the root
	 *         Data Set, if this is a Nested Data Set containing in a Sequence
	 *         Eleme
	 */
	public String getHeaderStringValue(int tagNr) {
		try {
			/*
			 * Attributes elem = getObject(); elem.setSpecificCharacterSet(
			 * "ISO_IR 100");
			 */
	...
	/**
	 * retrieves a specific HeaderTag that is inside anotehr tag or
	 * "0008,0102, 0054,0220" to get the Coding Scheme Designator after View
	 * Code Sequence
	 * 
	 * @return String * @param tagHierarchy; e.g. {Tag.UID, Tag.SOPInstanceUID,
	 *         Tag.CodeMeaning}
	 * @return
	 */
	...
	/**
	 * converts the int representation of a header number e.g. 0x00080010 to the
	 * corresponding String 0008,0010
	 * 
	 * @param  //headerNr
	 *            e.g. 0x00080010 as int
	 * @return 0008,0010 as String
	 */
	public static String toTagString(int tagNr) {

		return shortToHex(tagNr >> 16) + ',' + shortToHex(tagNr);
	}
	 ...
	 * Sequence seq= dicom.newSequence(Tag.AnatomicRegionSequence,0); Attributes
	 * dicom2 = new Attributes();
	 * 
	 * @param dicom
	 */
